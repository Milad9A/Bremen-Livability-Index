...
void main() {
  group('AuthBloc', () {
    ...
    group('PhoneSignInRequested', () {
      blocTest<AuthBloc, AuthState>(
        'emits verification id on code sent',
        setUp: () {
          when(
            mockAuthService.sendPhoneVerification(
              '+1234567890',
            ...
            onCodeSent('verification-id-123');
          });
        },
        build: () => AuthBloc(authService: mockAuthService),
        act: (bloc) =>
            bloc.add(const AuthEvent.phoneSignInRequested('+1234567890')),
        expect: () => [
          predicate<AuthState>((state) => state.isLoading == true),
          predicate<AuthState>(
            (state) =>
                state.isLoading == false &&
                state.phoneVerificationId == 'verification-id-123',
          ),
        ],
      );
    });
    ...
    group('PhoneCodeVerified', () {
      final phoneUser = testUser.copyWith(provider: AppAuthProvider.phone);

      blocTest<AuthBloc, AuthState>(
        'emits authenticated state on success',
        setUp: () {
          when(
            mockAuthService.verifyPhoneCode('verification-id', '123456'),
          ).thenAnswer((_) async => phoneUser);
        },
        build: () => AuthBloc(authService: mockAuthService),
        act: (bloc) => bloc.add(
          const AuthEvent.phoneCodeVerified('verification-id', '123456'),
        ),
        expect: () => [
          predicate<AuthState>((state) => state.isLoading == true),
          predicate<AuthState>(
      ...
      blocTest<AuthBloc, AuthState>(
        'emits error on wrong code',
        setUp: () {
          when(
            mockAuthService.verifyPhoneCode('verification-id', 'wrong'),
          ).thenThrow(Exception('Invalid code'));
        },
        build: () => AuthBloc(authService: mockAuthService),
        act: (bloc) => bloc.add(
          const AuthEvent.phoneCodeVerified('verification-id', 'wrong'),
        ),
        expect: () => [
          predicate<AuthState>((state) => state.isLoading == true),
          predicate<AuthState>(
