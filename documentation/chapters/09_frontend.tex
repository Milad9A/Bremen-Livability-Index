...
(vgl.\ Abschnitt~\ref{sec:favorites}) -- zweistufig:

\begin{itemize}
  \item \textbf{Gäste}: Die Einstellungen werden als JSON-String lokal
        im Gerätespeicher über \texttt{SharedPreferences} unter dem
        Schlüssel \texttt{user\_preferences} abgelegt.
  \item \textbf{Angemeldete Nutzer}: Die Einstellungen werden zusätzlich
        in Firestore unter
        \path{users/{user_id}/preferences/factor_settings} gespeichert
        und über einen Echtzeit-Stream (\texttt{watchCloudPreferences})
        geräteübergreifend synchronisiert. Beim Abmelden werden die
        Cloud-Präferenzen automatisch gelöscht.
\end{itemize}

Der \texttt{PreferencesBloc} koordiniert Lade- und Speichervorgänge
und stellt über \texttt{PreferencesLoaded} den aktuellen
\texttt{UserPreferences}-Zustand bereit, auf den \texttt{MapBloc}
bei jeder Score-Anfrage zugreift.
