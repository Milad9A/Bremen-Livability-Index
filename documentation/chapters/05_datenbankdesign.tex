...
Listing~\ref{lst:orm_example} zeigt die Basisklasse und ein konkretes Modell
als Beispiel:

\begin{lstlisting}[style=python, caption={ORM-Basisklasse und Beispielmodell \texttt{Tree}}, label={lst:orm_example}]
class GISBase(SQLModel):
    """Base class for all 21 GIS models."""
    class Config:
...
class Tree(GISBase, table=True):
    ...
    geometry: Any = Field(
        sa_column=Column(Geography("POINT", srid=4326))
    )
