...
\begin{enumerate}
      ...
      \item \textbf{Web-Deployment}: Render.com erkennt den Merge über einen
            GitHub-Webhook. Dank der \texttt{rootDir}-Konfiguration in
            \texttt{render.yaml} wird nur der Service neu gebaut, dessen
            Verzeichnis von der Änderung betroffen ist -- unabhängig von
            den Testergebnissen:
            \begin{itemize}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    % Row 1: Path split
    \node[path] at (-4.5, -1.6) (bpath) {\texttt{backend/**}};
    \node[path] at ( 4.5, -1.6) (fpath) {\texttt{frontend/**}};

    % Row 2: Tests + Deploys
    \node[test]   at (-6.5, -3.4) (bt) {\texttt{backend-tests.yml}};
    \node[deploy] at (-2.5, -3.4) (rb) {Render.com\\{\scriptsize Backend (Docker)}};
    \node[test]   at ( 2.5, -3.4) (ft) {\texttt{frontend-tests.yml}};
    \node[deploy] at ( 6.5, -3.4) (rf) {Render.com\\{\scriptsize Frontend (Static)}};

    % Row 3: Outcomes
    \node[codecov] at (-6.5, -5.2) (cov1) {Codecov\\(\texttt{backend})};
    \node[codecov] at ( 2.5, -5.2) (cov2) {Codecov\\(\texttt{frontend})};

    % Row 4: Build release
    \node[deploy] at ( 6.5, -6.8) (release)
      {\texttt{build-release.yml}\\{\scriptsize GitHub Release (4 Plattf.)}};

    % Arrows: Push → Paths
    \draw[arr] (push) -- (bpath);
    \draw[arr] (push) -- (fpath);

    % Arrows: Backend path → tests + deploy
    \draw[arr] (bpath) -- (bt);
    \draw[arr] (bpath) -- (rb);

    % Arrows: Frontend path → tests + deploy
    \draw[arr] (fpath) -- (ft);
    \draw[arr] (fpath) -- (rf);
    ...
    \draw[arr] (bt) -- (cov1);
    \draw[arr] (ft) -- (cov2);
    \draw[arr] (ft) -- node[pathlbl, right] {workflow\_run} (release);
  \end{tikzpicture}
  \caption{CI/CD-Pipeline: Pfadbasierte Auslösung nach dem Merge}
