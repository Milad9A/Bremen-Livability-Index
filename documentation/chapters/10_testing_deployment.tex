...
Bei jedem Merge auf \texttt{master} greift folgende Pipeline:

\begin{enumerate}
      \item \textbf{Parallele CI-Checks}: \texttt{backend-tests.yml} und
            \texttt{frontend-tests.yml} laufen unabhängig voneinander. Schlägt
            einer der Checks fehl, wird kein Deployment ausgelöst.
      \item \textbf{Web-Deployment}: Render.com erkennt den Merge automatisch
            ...
            Eingriff.
      \item \textbf{Native Builds}: Nach erfolgreichem Abschluss von
            \texttt{frontend-tests.yml} startet \texttt{build-release.yml}
            automatisch. Er baut in parallelen Jobs Android (APK), Windows
            (ZIP), Linux (tarball) und macOS (ZIP) und fasst alle Artefakte in
            einem einzigen GitHub~Release zusammen.
\end{enumerate}
