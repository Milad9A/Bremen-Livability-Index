...
class UserCreateRequest(BaseModel):
    ...
    provider: str = Field(
        ..., description="Auth provider: google, github, email, guest"
    )
    ...
    try:
        ...
        if existing_user:
            ...
            existing_user.display_name = (
                request.display_name or existing_user.display_name
            )
...
async def get_user_favorites(user_id: str, session: Session = Depends(get_session)):
    ...
    try:
        ...
        statement = (
            select(FavoriteAddress)
            .where(FavoriteAddress.user_id == user_id)
...
@app.post(
    "/users/{user_id}/favorites", response_model=FavoriteResponse, status_code=201
)
...
async def delete_favorite(
    user_id: str, favorite_id: int, session: Session = Depends(get_session)
):
    ...
    try:
        ...
        if favorite.user_id != user_id:
            raise HTTPException(
                status_code=403, detail="Favorite does not belong to this user"
            )
...
async def analyze_location(
    request: LocationRequest, session: Session = Depends(get_session)
):
    ...
    try:
        ...
        preferences_dict = (
            request.preferences.to_dict() if request.preferences else None
        )
        ...
        if is_enabled("greenery"):
            trees = fetch_nearby_features(
                session, Tree, point, LivabilityScorer.GREENERY_RADIUS, "tree"
            )
            nearby_features["trees"] = trees
            tree_count = len(trees)

            parks = fetch_nearby_features(
                session, Park, point, LivabilityScorer.GREENERY_RADIUS, "park"
            )
        ...
        if is_enabled("accidents"):
            accidents = fetch_nearby_features(
                ...
                Accident,
                point,
                LivabilityScorer.ACCIDENT_RADIUS,
                "accident",
                name_field="severity",
            )
        ...
        if is_enabled("public_transport"):
            transport = fetch_nearby_features(
                ...
                PublicTransport,
                point,
                LivabilityScorer.PUBLIC_TRANSPORT_RADIUS,
                "public_transport",
        ...
        if is_enabled("healthcare"):
            healthcare = fetch_nearby_features(
                ...
                Healthcare,
                point,
                LivabilityScorer.HEALTHCARE_RADIUS,
                "healthcare",
        ...
        if is_enabled("industrial"):
            industrial = fetch_nearby_features(
                ...
                IndustrialArea,
                point,
                LivabilityScorer.INDUSTRIAL_RADIUS,
                "industrial",
            )
        ...
        if is_enabled("major_roads"):
            roads = fetch_nearby_features(
                ...
                MajorRoad,
                point,
                LivabilityScorer.MAJOR_ROADS_RADIUS,
                "major_road",
        ...
        if is_enabled("bike_infrastructure"):
            bike_infra = fetch_nearby_features(
                ...
                BikeInfrastructure,
                point,
                LivabilityScorer.BIKE_INFRASTRUCTURE_RADIUS,
                "bike_infrastructure",
        ...
        if is_enabled("education"):
            education = fetch_nearby_features(
                ...
                Education,
                point,
                LivabilityScorer.EDUCATION_RADIUS,
                "education",
        ...
        if is_enabled("sports_leisure"):
            sports_leisure = fetch_nearby_features(
                ...
                SportsLeisure,
                point,
                LivabilityScorer.SPORTS_LEISURE_RADIUS,
                "sports_leisure",
        ...
        if is_enabled("pedestrian_infrastructure"):
            pedestrian = fetch_nearby_features(
                ...
                PedestrianInfrastructure,
                point,
                LivabilityScorer.PEDESTRIAN_INFRA_RADIUS,
        ...
        if is_enabled("cultural"):
            cultural = fetch_nearby_features(
                ...
                CulturalVenue,
                point,
                LivabilityScorer.CULTURAL_RADIUS,
                "cultural_venue",
        ...
        if is_enabled("railway"):
            railways = fetch_nearby_features(
                ...
                Railway,
                point,
                LivabilityScorer.RAILWAY_RADIUS,
                "railway",
        ...
        if is_enabled("gas_station"):
            gas_stations = fetch_nearby_features(
                ...
                GasStation,
                point,
                LivabilityScorer.GAS_STATION_RADIUS,
                "gas_station",
            )
        ...
        if is_enabled("parking"):
            parking = fetch_nearby_features(
                ...
                ParkingLot,
                point,
                LivabilityScorer.PARKING_RADIUS,
                "parking_lot",
        ...
        if is_enabled("airport"):
            airports = fetch_nearby_features(
                ...
                Airport,
                point,
                LivabilityScorer.AIRPORT_RADIUS,
                "airport",
        ...
        if is_enabled("construction"):
            construction = fetch_nearby_features(
                ...
                ConstructionSite,
                point,
                LivabilityScorer.CONSTRUCTION_RADIUS,
                "construction_site",
            )
