...
class TestScoringConstants:
    ...
    def test_positive_weights_sum(self):
        """Test that positive factors can yield up to 60 points."""
        max_positive = (
            LivabilityScorer.calculate_greenery_score(1000, 10)
            + LivabilityScorer.calculate_amenities_score(1000)
            + LivabilityScorer.calculate_public_transport_score(1000)
            + LivabilityScorer.calculate_healthcare_score(1000)
            + LivabilityScorer.calculate_bike_infrastructure_score(1000)
            + LivabilityScorer.calculate_education_score(1000)
            + LivabilityScorer.calculate_sports_leisure_score(1000)
            + LivabilityScorer.calculate_pedestrian_infra_score(1000)
            + LivabilityScorer.calculate_cultural_score(1000)
        )
        assert max_positive == 60.0

    def test_penalty_weights_sum(self):
        """Test that all penalties together total 57 points."""
        max_penalty = (
            LivabilityScorer.calculate_accident_penalty(1000)
            + LivabilityScorer.calculate_industrial_penalty(True)
            + LivabilityScorer.calculate_major_roads_penalty(True)
            + LivabilityScorer.calculate_noise_penalty(1000)
            + LivabilityScorer.calculate_railway_penalty(True)
            + LivabilityScorer.calculate_gas_station_penalty(True)
            + LivabilityScorer.calculate_waste_penalty(True)
            + LivabilityScorer.calculate_power_penalty(True)
            + LivabilityScorer.calculate_parking_penalty(True)
            + LivabilityScorer.calculate_airport_penalty(True)
            + LivabilityScorer.calculate_construction_penalty(True)
        )
        assert max_penalty == 57.0
...
class TestCalculateScore:
    ...
    def test_factors_list_contains_greenery(self):
        ...
        result = LivabilityScorer.calculate_score(
            tree_count=10, park_count=1, amenity_count=5, accident_count=0
        )
    ...
    def test_summary_generated(self):
        ...
        result = LivabilityScorer.calculate_score(
            tree_count=100, park_count=5, amenity_count=30, accident_count=0
        )
    ...
    def test_summary_mentions_greenery_when_high(self):
        ...
        result = LivabilityScorer.calculate_score(
            tree_count=100, park_count=5, amenity_count=0, accident_count=0
        )
