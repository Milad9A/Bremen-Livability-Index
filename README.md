...
1. **Web**: Uses Firebase OAuth popup authentication (configured in `lib/firebase_options.dart`)
2. **Mobile**: Configuration is in `lib/firebase_options.dart` (auto-generated by FlutterFire CLI)
3. **Android Release Signing**: Production builds are signed via GitHub Actions using repository secrets

Authentication providers: Google, GitHub, Email, Phone, Anonymous (Guest)
